...
[![364 tests](https://img.shields.io/badge/tests-364-brightgreen)](https://github.com/coreystevensdev/fairline/actions)
...
flowchart TD
    ...
    H --> I[User reviews candidates via GET /api/runs/id]
    I --> J[POST /api/runs/id/approve]
    J --> K[validate_agent: record CLV baseline]
...
The same NFL engine also carries home/away, primetime, weather, and surface splits, pulled from the same nflverse games CSV `fairline backfill-nfl` already downloads for scores and closing lines, so nothing new gets fetched or installed. Home/away and primetime read directly off the game row (weekday plus kickoff hour: Monday or Thursday, or Sunday at 19:00 or later); weather flags a game as bad when wind hits 15 mph or temperature drops to 32F or below, and is dropped from the split entirely on games with no recorded temp or wind, typically domes; surface only shows up in the rationale once the upcoming opponent's home surface is known, comparing history on that same surface against the line.

NFL also gets a `position_matchup` split, the same defense-vs-position idea as the NBA engine above but built on nflverse's own row-level `position` field (`QB`, `RB`, `WR`, `TE`) instead of a separate roster call. That field was already being read to filter which rows to keep during ingestion; it's now also stored on `PlayerGame` so the split can query it later. Where `vs_opponent` is one player's own history against the upcoming opponent, `position_matchup` pools every player at that position who has faced them, so a receiver with only one or two career games against a given defense still gets a matchup read built from every receiver who has played that defense, not just his own thin sample.
...
NHL gets the same shrinkage-and-market-bound treatment as MLB and NFL, on splits that fit hockey's own schedule and roster shape rather than reusing either sport's set as-is. MoneyPuck was the first data source considered and got ruled out during research: it publishes only per-player season CSVs, not a bulk per-game skater file, and its terms post an explicit anti-scraping stance. The NHL's own API is free, needs no key, and returns per-game boxscores directly, so that's the source. Seed per-game skater stats, then scan:

```bash
python -m fairline backfill-nhl-players --team EDM --season 20252026
python -m fairline matchup --sport icehockey_nhl --markets player_goals,player_assists,player_points,player_shots_on_goal
```
...
The `sim_agent` node computes probabilities per sport before picks are generated, consuming everything upstream context provides: results-fit ratings, current-season scoring rates, kickoff-hour wind, and rest spots (NBA back-to-backs cost about two points of expected margin, NFL short weeks one, bounded at three). NFL and NBA use a Normal margin family (Elo-style points ratings, sigma 13.5 and 11.5); NHL and MLB are low-scoring count sports, so they use a double-Poisson scoring model built from team rates, with regulation ties split by relative strength since neither sport can end tied, and NHL back-to-backs shaving a quarter goal off the tired side. For NFL: Elo-style points-scale team ratings fit from stored game results (home-field advantage 2.0, ratings regress a third toward zero between seasons), a Normal margin model (sigma 13.5) for win and cover probabilities, and a totals model built from current-season team scoring rates (Normal, sigma 10) for over/under probabilities. All arithmetic is code over data; Claude is never asked to estimate a probability. Seed the ratings with history in one command:
...
31. **NFL's `position_matchup` split is bucketed by the player's own most-recently-recorded `PlayerGame.position`, not by which prop market is being bet.** A traditional RB used as a receiver on a given slate is still pooled with running backs, not wide receivers, since nflverse's own position label is what gets stored.
