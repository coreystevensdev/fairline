...
import steambot.db.session as db_session
from steambot.api.main import resolve_session_factory
...
async def test_production_without_database_url_refuses_boot(monkeypatch, no_cached_engine):
    monkeypatch.setenv("STEAMBOT_ENV", "production")
...
async def test_dev_without_database_url_returns_none(monkeypatch, no_cached_engine):
    monkeypatch.delenv("STEAMBOT_ENV", raising=False)
    monkeypatch.delenv("DATABASE_URL", raising=False)
