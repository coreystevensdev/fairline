...
def _position_from_games(games: list[PlayerGame]) -> str | None:
    """The player's own most-recently-recorded position (most recent wins),
    read from an already-fetched games list instead of a fresh query -- the
    batched game fetch in create_matchup_candidates already has every row
    this would query for."""
    with_position = [g for g in games if g.position is not None]
    if not with_position:
        return None
    return max(with_position, key=lambda g: (g.season, g.week)).position
...
async def create_matchup_candidates(
    ...
    players_needed = {
        ...
        if (market, player, point) in fair_by_key and market in PROP_STAT_COLUMNS
    }
    ...
    async with session_factory() as session:
        ...
        games_by_player: dict[str, list[PlayerGame]] = {}
        if players_needed:
            rows = (
                (
                    await session.execute(
                        select(PlayerGame).where(
                            PlayerGame.player.in_(players_needed),
        ...
        for bm_key, pairs in retail_pairs.items():
            for (market, player, point), pair in pairs.items():
                ...
                # Splits don't depend on side (Over/Under share the same line and
                # the same game history), so compute them once per player/market
                # instead of re-sorting the same games list for each side.
                splits = compute_prop_splits(
                    games, stat, point,
