...
async def create_nhl_matchup_candidates(session_factory, snapshot, min_edge: float = 0.03) -> int:
    ...
    players_needed = {
        ...
        if (market, player, point) in fair_by_key and market in NHL_PROP_STAT_COLUMNS
    }

    created = 0
    async with session_factory() as session:
        games_by_player: dict[str, list[NhlPlayerGame]] = {}
        if players_needed:
            rows = (
                (
                    await session.execute(
                        select(NhlPlayerGame).where(NhlPlayerGame.player.in_(players_needed))
                    )
                )
                .scalars()
                .all()
        ...
        for bm_key, pairs in retail_pairs.items():
            for (market, player, point), pair in pairs.items():
                ...
                # skater/market instead of re-sorting the same games list twice.
                splits = compute_nhl_prop_splits(games, stat, point)
